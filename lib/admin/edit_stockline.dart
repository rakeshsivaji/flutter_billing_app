...
class Edit_StockLine extends StatefulWidget {
  const Edit_StockLine({super.key});

  @override
  State<Edit_StockLine> createState() => _Edit_StockLineState();
}

class _Edit_StockLineState extends State<Edit_StockLine> {
  ...
  void getData() async {
    dynamic response =
        await CommonService().getEditline(Get.arguments['id'].toString());
    if (response['status'] == 1) {
      setState(() {
        lineeditModel = LineeditModel.fromJson(response);
        name.text = lineeditModel!.data.name;
        selectedValue = lineeditModel!.data.status.toString();
      });
    }
    print(response);
  }
    ...
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        dynamic response = await CommonService()
            .updateline(lineeditModel!.data.lineId.toString(), newdata);
        if (response['status'] == 1) {
